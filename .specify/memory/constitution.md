...
- 版本变更：1.2.1 -> 1.3.0
- 新增原则：
  - VI. 仓库编码规范（从 CLAUDE.md 仓库约定章节形式化）
  - VII. 设计系统视觉规范（从 CLAUDE.md 设计风格章节与 design-style.md 形式化）
- 修改的原则：
  - 无
- 移除章节：
  - 无
- 需要同步更新的模板：
  - ✅ .specify/templates/plan-template.md（宪章检查补充 VI、VII 合规项）
  - ✅ .specify/templates/spec-template.md（FR-008 引用设计系统变体模型与数值约束）
  - ✅ .specify/templates/tasks-template.md（打磨阶段补充设计系统合规验证任务）
  - ⚠ .specify/templates/agent-file-template.md（下次 CLAUDE.md 重新生成时应引用 VI、VII）
- 后续 TODO：
  - 无
-->
...
**Ark UI 行为基础层**：凡是具有复杂交互行为（浮层定位、焦点管理、状态机、ARIA 输出）的组件，必须优先基于 Ark UI（`@ark-ui/react`）构建，而不是手工实现相同能力。样式层保持使用 CSS Modules（Less）+ 设计 token，不依赖 Ark UI 的默认样式。公开 API 必须与 Ark UI 内部接口解耦。非 click 触发类型应通过受控模式（`open` prop）桥接。参考实现：`packages/components/src/popover/index.tsx`。
...
凡是预期会复用的视觉决策，都必须通过共享设计 tokens 与主题 primitives 表达。组件必须消费规范化的 tokens 来处理颜色、排版、间距、圆角、动效和状态样式，而不能直接写死一次性的值。任何新的视觉 primitive 都必须文档化其浅色/深色主题行为、密度影响以及可覆盖边界。`@deweyou-ui/styles` 是所有 `--ui-*` 变量的唯一事实来源；组件直接消费这些变量，不得内联 token 值。理由：以 token 驱动的样式系统可以让组件库保持一致、可主题化，并能安全扩展。
...
**源码单元结构**：`packages/components`、`packages/hooks`、`packages/utils` 中，
...
设计系统数值合规性（原则 VII），以及 `/specs/` 下文档语言是否符合简体中文要求。

**版本**： 1.3.0 | **批准日期**： 2026-03-15 | **最后修订**： 2026-04-08
