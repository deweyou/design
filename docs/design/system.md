...
- **宋体是品牌身份**：body 和 display 都使用宋体/Serif。拉丁字母、数字在宋体下呈现衬线感是刻意选择，不要在组件里局部切换成 sans-serif。
- **语义少于装饰**：全局只承认 neutral、primary、danger 三个组件语义色。色彩用于表达角色，不用于制造热闹。
- **暖白画布承载内容**：页面底色是带温度的浅色画布，不是纯白营销页；深色主题也应像纸面墨色，而不是高对比发光界面。
...
英文 eyebrow 或微标签可以使用 uppercase + letter-spacing，但只用于很小的辅助层级。正文、标题、组件文字仍应保持宋体。
...
浅色主题使用暖白或轻微灰绿的 canvas；surface 和 raised surface 逐级抬高。不要使用纯白作为页面默认底色，也不要用大面积渐变承载页面气氛。
...
| focus    | 仅 `:focus-visible`，双层 `box-shadow`，不使用 `outline` 或改 border                        |
...
- focus 是否只在 `:focus-visible` 下出现，并使用双层 `box-shadow`？
- loading 是否保留布局，不造成按钮或文字跳动？
- 图标是否来自 `@deweyou-design/react-icons`？
- 文案是否简体中文优先、技术性、克制、无 emoji？
- website 和 H5 是否复用同一套理念，而不是另起一套移动端视觉？

_Last updated: 2026-05-09 | Reason: synced Claude Design handoff philosophy into the repository design-system knowledge base._
