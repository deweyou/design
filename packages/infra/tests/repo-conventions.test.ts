...
test('repository guidance documents the shared authoring conventions', () => {
  ...
  const componentAgents = read('packages/react/CLAUDE.md');
  const hookAgents = read('packages/react-hooks/CLAUDE.md');
  const utilAgents = read('packages/infra/CLAUDE.md');
  ...
  expect(utilAgents).toContain('`packages/infra/tests` 下的顶层测试');
