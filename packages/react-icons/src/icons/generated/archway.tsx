// This file is generated by scripts/generate-icons.mjs.
// Do not edit directly. Update src/icon-registry/icons.json or src/icon-registry/assets/*.svg, then run pnpm run generate-icons.
import { createIcon } from '../../icon-wrapper';

export const ArchwayIcon = /* @__PURE__ */ createIcon('ArchwayIcon', {
  viewBox: '0 0 24 24',
  body: (
    <>
      <g id="deweyou-icon-tdesign-archway-archway">
        <path
          id="deweyou-icon-tdesign-archway-fill1"
          fill="transparent"
          d="M4 21V9H20V21H4Z"
        ></path>
        <path
          id="deweyou-icon-tdesign-archway-fill2"
          fill="transparent"
          d="M5 4H19L20 9H4L5 4Z"
        ></path>
        <path
          id="deweyou-icon-tdesign-archway-stroke1"
          stroke="currentColor"
          d="M19 4H5M19 4L20 9M19 4H20M19 4V3M5 4L4 9M5 4H4M5 4V3M4 9V21M4 9H3M4 9H9M4 21H3M4 21H9M20 9V21M20 9H21M20 9H15M20 21H21M20 21H15M9 21V9M9 21H15M9 9H15M15 9V21"
          strokeLinecap="square"
          strokeWidth="2"
        ></path>
      </g>
    </>
  ),
});
