...
| focus    | emerald 焦点环，仅键盘触发 | `:focus-visible { box-shadow: 0 0 0 2px var(--ui-color-surface), 0 0 0 4px var(--ui-color-focus-ring); outline: none; }` |

### 焦点规范

- 焦点环使用 `box-shadow` 实现，兼容 `border-radius`，不用 `outline` 或 `border` 模拟
- 必须使用 `:focus-visible` 而非 `:focus`，确保鼠标点击不触发焦点环

```less
// ✓ box-shadow 实现（兼容 border-radius）
.root:focus-visible {
  box-shadow:
