...
> 本节由 `/harness-dev` archive 步骤自动追加。
> 记录跨 feature 的可泛化洞察，供后续开发决策参考。

<!-- harness-dev archive step appends entries here -->

### 20260409-component-testing-standards — 2026-04-09

- Type: feature
- Key findings:
  - `@vitest/coverage-v8` must be pinned to match the vitest version bundled inside the test framework (vite-plus-test uses `4.1.x`); mismatched versions silently break coverage collection with a cryptic `fetchCache` error.
  - Coverage measurement must run from inside `packages/react`, not the workspace root — root-level runs aggregate all packages and dilute component coverage to ~50%.
  - jsdom-hostile branch categories (SSR guards, ResizeObserver layout, async blur/focus timeouts, complex ref callbacks) are genuinely untestable at ≥80% threshold; accept these as known gaps rather than writing fragile workarounds.
- Promoted: `knowledge/testing-standards.md` (full testing standards reference for component authors)
